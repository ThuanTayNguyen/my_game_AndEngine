...
public class Level3_Activity extends SimpleBaseGameActivity {
    ...
    private String path="level3/level3.tmx";
    ...
    // dongho
    private LinkedList<Sprite> NgoiSaoArray;
    private BitmapTextureAtlas NgoiSaoBitMap;
    private TextureRegion NgoiSaoRegion;
    ...
    private int time=40;
    //    vat can
    private LinkedList<Sprite> vatCanArray;
    private BitmapTextureAtlas vatCanBitMap;
    private TextureRegion vatCanTextureRegion;
    ...
    private void onLoadNgoiSao() {
        BitmapTextureAtlasTextureRegionFactory.setAssetBasePath("tmx/level2/");
        NgoiSaoBitMap=new BitmapTextureAtlas(this.getTextureManager(),32,32,TextureOptions.BILINEAR);
        NgoiSaoRegion=BitmapTextureAtlasTextureRegionFactory.createFromAsset(NgoiSaoBitMap,this,"star.png",0,0);
        NgoiSaoBitMap.load();

    }
    private void onLoadVatCan() {
        BitmapTextureAtlasTextureRegionFactory.setAssetBasePath("tmx/level3/");
        vatCanBitMap=new BitmapTextureAtlas(this.getTextureManager(),32,32,TextureOptions.BILINEAR);
        vatCanTextureRegion=BitmapTextureAtlasTextureRegionFactory.createFromAsset(vatCanBitMap,this,"Chuongngaivat.png",0,0);
        vatCanBitMap.load();
    }
    ...
    private void lever3() {
        ...
        for(final TMXObjectGroup group:this.mTMXTiledMap.getTMXObjectGroups()){
            for (final  TMXObject object:group.getTMXObjects()){
                if (group.getName().equals("groundPhysic")){
        ...
        NgoiSaoArray=new LinkedList<>();
        for(final TMXObjectGroup group:this.mTMXTiledMap.getTMXObjectGroups()){
            for (final  TMXObject object:group.getTMXObjects()){
                if (group.getName().equals("Star")){
                    Sprite donghoSprite=new Sprite(object.getX(),
                            object.getY(),object.getWidth(),
                            object.getHeight(),
                            NgoiSaoRegion,
                            getVertexBufferObjectManager());
                    NgoiSaoArray.add(donghoSprite);
                }
            }
        }
        vatCanArray =new LinkedList<>();
        for(final TMXObjectGroup group:this.mTMXTiledMap.getTMXObjectGroups()){
            for (final TMXObject object:group.getTMXObjects()){
                if(group.getName().equals("Obs")){
                    Sprite vatcanSprite= new Sprite(object.getX(),
                            object.getY(),object.getWidth(),
                            object.getHeight(),vatCanTextureRegion,getVertexBufferObjectManager());
                    vatCanArray.add(vatcanSprite);
                }
            }
        }
//        dem so luong chuot.
        for(int i=0;i<chuotArray.size();i++) {
            Score_nextlever++;
            mScene.attachChild(chuotArray.get(i));
        }
        for(int i=0;i<NgoiSaoArray.size();i++) {
            mScene.attachChild(NgoiSaoArray.get(i));
        }
        for (Sprite s:vatCanArray){
...
//        this.mCAMERA.setBoundsEnabled(true);
    ...
    public void onBackPressed() {
        Intent intent =new Intent(Level3_Activity.this,MenuGameActivity.class);
        ...
        try {
            runOnUiThread(new Runnable() {
                ...
                public void run() {
                    ...
                    final EditText myEditText=new EditText(Level3_Activity.this);
                            ...
                            new DialogInterface.OnClickListener() {
                                ...
                                public void onClick(DialogInterface dialogInterface, int i) {
                                    try {
                                        ...
                                        editMyNameShared.putString("KEY_NAME",yourname+" (Level 3)");
                                        editMyScoreShared.putInt("KEY_SCORE",myScore);
                                        editMyScoreShared.commit();
                                        editMyNameShared.commit();
                                        Log.v("Gui","Đã gửi được" +yourname);
                                        Intent intent =new Intent(Level3_Activity.this,HighScoreActivity.class);
                                        startActivity(intent);
                                        finish();
        ...
        try {
            runOnUiThread(new Runnable() {
                ...
                public void run() {
                    ...
                    final EditText myEditText = new EditText(Level3_Activity.this);
                    builder.setView(myEditText);
                    builder.setMessage("Chúc mừng ! Bạn là người thắng cuộc ! " + " Tên bạn là gì ?").setCancelable(true).setPositiveButton("Lưu",
                            new DialogInterface.OnClickListener() {
                                @Override
                                public void onClick(DialogInterface dialogInterface, int i) {
                                    try {
